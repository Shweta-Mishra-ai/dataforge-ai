...
st.caption("Auto-cleaning pipeline — every action logged and explained.")
st.divider()
...
with st.spinner("Running cleaning pipeline..."):
    ...
    st.session_state["df_active"] = df_clean

with st.spinner("Running statistical analysis..."):
    stats = run_stats(df_clean.to_json())

# ══════════════════════════════════════════════════════════
#  SECTION 1 — BEFORE vs AFTER SNAPSHOT
# ══════════════════════════════════════════════════════════
...
if summary["duplicates_removed"] > 0:
    with st.expander(
        "DUPLICATES REMOVED — {:,} rows".format(summary["duplicates_removed"]),
        expanded=True
    ):
        st.error(
            "**{:,} duplicate rows** were found and removed.\n\n"
            "Original: {:,} rows  →  Cleaned: {:,} rows".format(
...
if missing_actions:
    with st.expander(
        "MISSING VALUES HANDLED — {} column(s)".format(len(missing_actions)),
        expanded=True
    ):
        rows = []
...
if dropped:
    with st.expander(
        "COLUMNS DROPPED — {}".format(len(dropped)),
        expanded=False
    ):
        for a in dropped:
...
if flagged:
    with st.expander(
        "OUTLIERS FLAGGED — {} column(s)".format(len(flagged)),
        expanded=False
    ):
        st.info(
            "Extreme outliers (3x IQR) are **flagged but not removed** — "
...
if all_minor:
    with st.expander(
        "OTHER CHANGES — {}".format(len(all_minor)),
        expanded=False
    ):
        rows = []
        for a in all_minor:
...
# ══════════════════════════════════════════════════════════
#  SECTION 3 — REAL STATISTICAL ANALYSIS
# ══════════════════════════════════════════════════════════
...
with tab_cat:
    if not cat_cols:
        ...
        c1, c2 = st.columns(2)
        c1.info("**Low cardinality** (10 or fewer unique values) — Good for grouping and charts.")
        c2.warning("**High cardinality** (over 80% unique) — Likely an ID field, skip in analysis.")
...
with tab_corr:
    if not stats.correlations:
        ...
        if not sig_corr:
            ...
            c2.warning("**|r| 0.4 to 0.7** — Moderate, worth investigating")
...
# ══════════════════════════════════════════════════════════
#  SECTION 4 — PER-COLUMN DETAIL
# ══════════════════════════════════════════════════════════
st.markdown("### Column-by-Column Detail")
st.caption("Full breakdown for every column — missing values, outliers, distribution.")
...
for col in df_clean.columns[:25]:
    ...
    with st.expander(
        "{} {}  |  {}  |  Score: {}/100".format(
            score_tag, col, cs.dtype, int(score))
    ):
        ...
        if cs.missing_count > 0:
            st.warning("Missing: {} values ({:.1f}%) — handled in cleaning step.".format(
