...
def build_pdf(
    ...
    config: ReportConfig,
    chart_data: list,        # [(title, png_bytes), ...]
    chart_narratives: list,  # [narrative_str, ...]
    executive_summary: list, # [{finding, type}, ...]
    recommendations: dict,   # {immediate, short_term, long_term}
) -> bytes:
    ...
    def _header_footer(canvas, doc):
        ...
        # Header text right — report title
        canvas.setFont("Helvetica", 8)
        title_short = config.title[:50]
        canvas.drawRightString(W - m, H - 9*mm, title_short)
...
def _dataset_overview_page(story, styles, theme, df, profile):
    ...
    type_data.append([
        "Numeric", str(len(profile.numeric_cols)),
        ", ".join(profile.numeric_cols[:5]) +
        ("..." if len(profile.numeric_cols) > 5 else "")
    ])
    type_data.append([
        "Categorical", str(len(profile.categorical_cols)),
        ", ".join(profile.categorical_cols[:5]) +
        ("..." if len(profile.categorical_cols) > 5 else "")
    ])
    type_data.append([
        "DateTime", str(len(profile.datetime_cols)),
        ", ".join(profile.datetime_cols[:5]) +
        ("..." if len(profile.datetime_cols) > 5 else "")
    ])
    _render_table(story, theme, type_data,
...
def _data_quality_page(story, styles, theme, profile):
    ...
    hdr = ["Column", "Type", "Missing %", "Unique", "Outliers", "Score"]
    rows_ = [hdr]
...
def _metrics_page(story, styles, theme, df, profile):
    ...
    for col in num_cols:
        s = df[col].dropna()
        cards.append((col, f"{s.sum():,.0f}",
                      f"avg {s.mean():,.2f}  |  max {s.max():,.2f}"))
...
def _anomaly_page(story, styles, theme, df, profile):
    ...
    if not found_any:
        ok = Table(
            [[Paragraph("No significant outliers detected in any numeric column.", styles["body"])]],
            colWidths=[170*mm]
        )
...
def _recommendations_page(story, styles, theme, recs):
    ...
    for title, key, colour in sections:
        ...
        for item in items:
            row = Table([[
                Paragraph("[ ]", ParagraphStyle("cb",
                    fontSize=9, textColor=_rgb(colour),
                    fontName="Helvetica-Bold")),
...
def _appendix_page(story, styles, theme, config, profile):
    ...
    story.append(Paragraph(
        ...
        "with a 1.5x multiplier. AI narrative paragraphs are generated "
        "using Groq Llama 3.3 70B via the DataForge AI platform.",
        styles["body"]
    ))
    story.append(Spacer(1, 4*mm))
    ...
    story.append(Paragraph(
        ...
        f"All findings are based solely on the provided dataset and should "
        f"be verified by a qualified data analyst before making business "
        f"decisions. DataForge AI accepts no liability for decisions made "
...
def _build_styles(theme: ReportTheme) -> dict:
    T = theme
